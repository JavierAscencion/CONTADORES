...
int size_s;             // Tamaï¿½o de trama que espera leer el puerto serial (se utiliza para agilizar la captura de datos)
...
/////EEPROM///////////
const int word_size = 10; // TAMAï¿½O DE LOS DATOS EN EEPROM//antes 8
...
void reset();
//------------------------------------------------------------------------------
// Rutina de gestiï¿½n de interrupciones
//------------------------------------------------------------------------------
...
void main()
{
   ...
   enable_interrupts(INT_TIMER1);            // Habilito interrupciï¿½n particular del TIMER1
   ...
   while (TRUE)
   {
      ...
      {               // bloqueo de par 1 por mï¿½s de X segundos
      ...
      { // bloqueo de par 2 por mï¿½s de X segundos
         bloq_p2 = 0;
         if ((!en_b1) && (!en_b2) && (!en_b3))
         {
            lcd_gotoxy(10, 1); // cronometro para limpiar
      ...
      { // bloqueo de par 3 por mï¿½s de X segundos
         bloq_p3 = 0;
         if ((!en_b1) && (!en_b2) && (!en_b3))
         {
            lcd_gotoxy(10, 1); // cronometro para limpiar
      ...
      sensores(); // Revisa estado de los sensores
       ////////////////// verificar si ya contó el pasajero y envía pulso por relay //////////////////////////////////////////
